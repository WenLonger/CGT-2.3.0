...
$uslog = CGT($_COOKIE['uslog']);
$uspass = CGT($_COOKIE['uspass']);
...
mysql_query("UPDATE `users` SET `viz`='".time()."', `ip`='".CGT($_SERVER['REMOTE_ADDR'])."',`browser`='".CGT($_SERVER['HTTP_USER_AGENT'])."',`gde`='".CGT($_SERVER['REQUEST_URI'])."',`gdeon`='".CGT($_SERVER['SCRIPT_NAME'])."' WHERE `id`='".$users['id']."'");
...
$page = CGT(intval($k_page));
elseif(is_numeric($_GET['selection'])) 
$page = CGT(intval($_GET['selection']));
}
if ($page<1)$page=1;
