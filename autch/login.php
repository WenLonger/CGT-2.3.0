...
echo '<title>Авторизация - '.$index['pod-title'].'</title>';
echo '<meta name="description" content="Авторизация - '.$index['pod-title'].'">';
...
$pass = md5(md5(md5(CGT($_POST['pass'])))); //фильтрируем и в мд5
...
$dbsql_pass = mysql_fetch_array(mysql_query("SELECT `pass` FROM `users` WHERE `pass`='".$pass."' LIMIT 1"));
if($dbsql_pass == 0) {
