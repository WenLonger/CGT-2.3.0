...
$k_post = mysql_result(mysql_query("SELECT COUNT(*) FROM `forum_tema` WHERE `name` like '%".CGT($tema)."%'"),0);
if ($k_post==0){
...
echo'<div class="ok" style="background: none; border-radius: 0px; color: #d6d6d6; border-bottom: 1px solid #2d2d2d; padding: 5px 10px; margin-bottom: 10px;">По запросу <b>#'.CGT($tema).'</b> найдены следующие результаты:</div>';
}

$q = mysql_query("SELECT * FROM `forum_tema` WHERE `name` like '%".CGT($tema)."%' ORDER BY `time` DESC");
