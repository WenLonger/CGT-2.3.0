...
if(isset($_REQUEST['okset'])) { $okback = CGT($_POST['okback']); mysql_query("UPDATE `users` SET `background` = '".$okback."' WHERE `id` = '".$user[id]."' LIMIT 1");}
if(isset($_REQUEST['okset'])) { $oksetbar = CGT($_POST['oksetbar']); mysql_query("UPDATE `users` SET `headback` = '".$oksetbar."' WHERE `id` = '".$user[id]."' LIMIT 1"); echo '
<script>
let stateObj = {
  foo: "bar",
};

history.pushState(stateObj, "page 2", "'.$HOME.'/user_'.$user['id'].'");
</script>
...
echo '<a class="bot_bal" href="'.$HOME.'/pay/" style="float: right;">Пополнить</a>';
echo '</div>';
} else {
echo '<div class="plus_money">';
